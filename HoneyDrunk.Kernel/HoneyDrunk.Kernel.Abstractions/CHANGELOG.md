...
- **GridContextSnapshot**: Abstractions-only initialized `IGridContext` implementation for transport envelopes, library-owned background scopes, and tests that must bootstrap context without a runtime `HoneyDrunk.Kernel` dependency.

### Changed

- **Identity parsing consolidation**: Centralized repeated ULID and kebab-case parsing/validation helper logic used by strongly typed identity primitives without changing public identity APIs.

## [0.6.0] - 2026-05-17

### ⚠️ Breaking Changes

- **WellKnownNodes canonical IDs**: Existing well-known Node IDs now use canonical `honeydrunk-*` values instead of short aliases. For example, `WellKnownNodes.Core.Kernel` is now `honeydrunk-kernel` and `WellKnownNodes.Ops.Pulse` is now `honeydrunk-pulse`.

### Added

- **Missing Grid Nodes**: Added well-known IDs for Vault.Rotation, Audit, Communications, Notify, Actions, Architecture, Studios, Lore, and the current AI-sector Nodes.
