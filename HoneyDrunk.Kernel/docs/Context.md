...
**Key Relationship (v0.3.0 Model):**
- `OperationContext` **is the span** - it owns `OperationId` (span-id) and represents "this specific unit of work"
- `GridContext` **is the ambient envelope** - carries `CorrelationId` (trace-id) and `CausationId` (parent-span-id) for propagation
- `OperationId` is **owned by OperationContext** - generated by `IOperationContextFactory`, NOT stored on GridContext
- `IGridContext` is a **pure value object** - no `OperationId`, no factory methods (moved to `IGridContextFactory`)

**Three-ID Model:**
1. **CorrelationId** (trace-id) - Owned by `IGridContext`, constant across entire trace
2. **OperationId** (span-id) - Owned by `IOperationContext`, unique per operation
3. **CausationId** (parent-span-id) - Owned by `IGridContext`, points to parent's OperationId
...
{
    string CorrelationId { get; }  // string, trace-id (constant across trace)
    string? CausationId { get; }   // string?, parent-span-id (points to parent OperationId)
    ...
    string? TenantId { get; }      // string?, optional tenant identifier
    string? ProjectId { get; }     // string?, optional project identifier
    ...
    // NOTE: OperationId is NOT on IGridContext - it's owned by IOperationContext
}
...
**Note:** `OperationId` (span-id) is **NOT on IGridContext** - it is owned by `IOperationContext`. See [IOperationContext](#ioperationcontextcs) for span-id access.
...
Child contexts are created using `IGridContextFactory`, not directly on `IGridContext`:

```csharp
public class OrderService(
    IGridContextFactory gridFactory,
    IOperationContext currentOperation,
    ILogger<OrderService> logger)
{
    public async Task ProcessOrderAsync(Order order)
    {
        ...
        var childContext = gridFactory.CreateChild(
            parent: currentOperation.GridContext,
            operation: currentOperation,
            nodeId: "payment-node");
        
        // Verify three-ID relationships:
        // childContext.CorrelationId == currentOperation.CorrelationId (same trace)
        // childContext.CausationId == currentOperation.OperationId (parent-child link)
        // Child's OperationContext will have new OperationId (new span)
...
public class OrderService(IGridContext gridContext, ILogger<OrderService> logger)
{
    ...
    {
        logger.LogInformation(
            "Processing order {OrderId} - Trace: {CorrelationId}",
            order.Id,
...
| `OperationId` | string | **This operation's unique span-id (CANONICAL OWNER - not pass-through)** |
...
**Ownership Model (v0.3.0):**
- `OperationId` is **owned and stored** by `IOperationContext` (not a pass-through from GridContext)
- `CorrelationId` and `CausationId` are **convenience properties** that pass through to `GridContext`
- This design keeps `GridContext` as a pure value object while giving `OperationContext` first-class span identity
...
- **Generates new `OperationId`** and stores it on the OperationContext (NOT on GridContext)
...
**Design (v0.3.0):** The factory generates `OperationId` and wires it into `OperationContext`. The `OperationId` is **NOT stored on GridContext** - GridContext remains a pure value object carrying only `CorrelationId` and `CausationId`. When creating child contexts across Node boundaries, use `IGridContextFactory.CreateChild(parent, operation)` which sets the child's `CausationId` to the parent operation's `OperationId`.
...
## Causation Chain Example (v0.3.0)

```
User Request (Entry Point)
  GridContext:      CorrelationId=ABC123, CausationId=null
  OperationContext: OperationId=OP-001, OperationName="HttpRequest"
    ↓
API Gateway creates child context via IGridContextFactory
  GridContext:      CorrelationId=ABC123, CausationId=OP-001
  OperationContext: OperationId=OP-002, OperationName="RouteRequest"
    ↓
Order Service creates child context
  GridContext:      CorrelationId=ABC123, CausationId=OP-002
  OperationContext: OperationId=OP-003, OperationName="ProcessOrder"
    ↓
Payment Service creates child context
  GridContext:      CorrelationId=ABC123, CausationId=OP-003
  OperationContext: OperationId=OP-004, OperationName="ChargePayment"
```

**Key Points:**
- **CorrelationId stays constant** (`ABC123`) throughout the entire request tree (trace-id)
- **OperationId is unique** per operation (`OP-001`, `OP-002`, etc.) and **owned by OperationContext** (span-id)
- **CausationId points to parent's OperationId** (parent-span-id) - stored on GridContext, forms the tree structure
